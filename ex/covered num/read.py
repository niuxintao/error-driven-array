...
#   fig, ax = plt.subplots()
    ...
    while line:
        line = line.strip()
        if len(line) != 0 and line[0] == '(' and i < 2:
            [x, y] = dealLine(line)
            ...
            if i == 0:
                lengthMin = len(x)
                rectc1 = show.show1(ax, x + bar_inc[i], y, a[i], color[i])
            elif i == 1:
                rectc2 = show.show1_special(ax, x + bar_inc[i], y, lengthMin, a[i], color[i])
            i = i + 1
        line = inf.readline()   
    inf.close()
...
if __name__ == "__main__":
    ...
    rectc1, rectc2 = readFile(ax7,"2-way gcc.txt")
