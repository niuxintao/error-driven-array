...
(2) \emph{MFS identification} : Traditional MFS identification aims at finding the MFS in a failing test case. As discussed before, test cases in the covering array are not enough to identify the MFS. Hence, additional test cases are should be generated and executed in this CT activity. Normally, an additional test case is generated based on the original failing test case. By comparing the difference between the additional test cases and the original failing test cases, the failure-inducing parts can be determined. Take the OFOT approach as an example. In Table \ref{tradition-gi}, by comparing the original failing test case $t_{1}$ and the additional passing test case $t_{11}*$, we can determine the (-, 0, -, -) is the only difference, which should be the failure part. So an additional test case is usually generated by mutating some parts of the original failing test case, and keeping the remaining parts. Formally, let $t_{failing}$ be the original failing test case, $\Delta$ be the mutation parts, $\mathcal{P}$ be the parameters and their values, then the traditional additional test case generation can be formulated as \ref{eq3}.
\begin{displaymath}t \leftarrow  mutant (\mathcal{P}, t_{failing}, \Delta )  \tag{EQ3} \label{eq3} \end{displaymath}

\ref{eq3} indicates that the test case $t$ are generated by mutating the part $\Delta$ of the original failing test case $t_{failing}$. The mutated values are , but not be the same as that in $t_{failing}$.
Normally,
\begin{displaymath}\mathcal{T}_{candidate} =  \{\ t\ |\ t \leftarrow  mutant (\mathcal{P}, t_{failing}, \Delta )\ \} \tag{EQ4} \label{eq4} \end{displaymath}
