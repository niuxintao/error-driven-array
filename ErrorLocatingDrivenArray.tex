...
Combinatorial testing is an important black-box testing technique, and have been widely applied in practice \cite{kuhn2010practical}. Particular testing scenarios are configuration testing, programming input testing and sequence testing. Changhai nie \cite{nie2011survey} comprehensively took a survey on the studies about different aspects of combinatorial testing, and based on which, we can learn that many works in CT focus on seperately aspects, i.e. either focus on generation, or on evaluation or on fault localization, or on building models .

To avoid this, 
the methodology, notions and in CT.
On this filed, most works separately discussed , such as.

JieLi \cite{li2012improved} that can utilize the previous test cases.  \cite{fouche2009incremental} introduce Incremental testing cases. \cite{yilmaz2013test} gives an test-aware testing.  \cite{bryce2006prioritized} gives both for generation and priozitation. \cite{dumlu2011feedback} gives and feedback . \cite{yilmaz2013reducing} based on this .

Aiming to avoid the 顽固的，一成不变的模式，some works made efforts to change the , i.e., introducing the adaptvie machine,.

Additionally, Changhai nie \cite{nie2013adaptive} introduce the notion of adaptive combinatorial testing.


Our work differs from them mainly at we proposed a new framework that tightly related the generation and localization in a more tiggt work. 
