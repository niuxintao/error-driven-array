...
To overcome such deficiencies in traditional CT, we propose a new CT generation-identification framework. Our new framework aims at enhancing the interaction of generation and identification to reduce the unnecessary and invalid test cases discussed previously. The basic outline of our framework is illustrated in Figure \ref{new-life}.
\begin{figure}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
     ...
     $S_{MFS}$ \Comment{the set of MFS that currently identified}
...
2) \emph{Identification} : %只选fixed，然后其余相似，然后重点说明一下fixed
...
\subsection{Compare with Traditional First-Generation-Then-Identification approach}
...
The result is listed in Table \ref{cm_elda_fglt}. In Column `Method', \emph{ncf} indicates our new CT framework and \emph{fgti} indicates the traditional first-generation-then-identification approach. The results of three coveing criteria, i.e., 2-way, 3-way, and 4-way are shown in three main columns. In each of them, the overall test cases (size), precise, recall, and f-measure are listed.
...
In summary, our approach needs much less test cases than traditional first-generation-then-identification approach, and there is no decline in the quality of the identified MFS when comparing with traditional approach.
...
As both the cost (number of test cases) and the quality of the identified schemas are important in practice. We combine the two metrics (\emph{size} and \emph{f-measure}) into a single one, by dividing \emph{f-measure} by \emph{size}. The normalized result of this combination metric is shown in Fig.\ref{cm_performance}. In this figure, \emph{ncf} and \emph{fglt} represent our new approach and traditional first-generation-then-identification approach. \emph{Ela} indicates the error locating array approach.
