...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
     ...
     \While{\textbf{not} $t_{uncovered}$ is not empty}
      \While{\textbf{not} $test$ is satisfied }
       \State $test \leftarrow emptyArray(Param.size)$
       \ForAll {$factor \in test$}
            \While{\textbf{not} $SAT-Check(test, S_{MFS})$}
              \State $value \leftarrow select(Param,opt) $
           \EndWhile
       \EndFor
       \EndWhile
...
%       \EndIf
    \EndWhile
     ...
     \State \Return \emph{test}
  \end{algorithmic}
\end{algorithm}

In this paper as we think to ,  is the most ada way for . We adopt the Need constraint.  Inspired by the Constraint AETG work by Cohen, we .

2) \emph{Isolation} :
Isolation is still the same as before: 1) the additional test case should not contain the MFS, if cannot find, set the test as . 2) when update the MFS set. and coverage set.
