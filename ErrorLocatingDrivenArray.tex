...
As discussed in Section \ref{sec:moti}, SCT is suffering from masking effects when there are multiple MFS in one failing test case. Since our approach theoretically reduce the probability of the case that multiple MFS appear in one test case, we believe our framework can alleviate the masking effects. In fact, our framework conforms to \emph{tested t-way interaction criterion} because we only update t-way coverage for two types of schemas : (1) $t$-degree schemas in those passing test cases and (2) $t$-degree schemas \emph{related} to MFS. Hence, our framework \emph{\textbf{Interleaving CT}}(ICT) supports a better adequate testing than SCT.
