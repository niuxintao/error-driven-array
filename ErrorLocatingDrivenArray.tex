...
\label{fda-cit-life}
\end{figure}
FDA-CIT \cite{yilmaz2013reducing} is a feedback framework that can augment the traditional covering array to iteratively identify the MFS, and can handle the masking effects. The overall process can be illustrated in Fig \ref{fda-cit-life}. Specifically, it will first generate a t-way covering array and executed all the test cases in it. After that it will utilize the classification tree method to identify the MFS. Then it will forbidden the identified MFS to be appeared and compute the tested-t-coverage. If the tested-t-coverage is not satisfied, it will repeat the previous process, i.e., generating additional test cases and identifying MFS. Like our ICT approach, FDA-CIT is also an adaptive approach that iteratively identify the MFS to reduce the possible masking effects among then and to obtain a more adequate testing. Besides these commonalities, there are several important difference between our approach and FDA-CIT (marked with darker background in Fig \ref{fda-cit-life}):

First, the granularity of adaptation. Our Framework are more  is . There is not to which are better or not.

Second, the MFS identification approach is different. 

Third, the coverage is not the same.


The main difference of FDA-CIT with our approach ICT are several components (in ).

First, it use classification tree to. Different from our OFOT approach, it does not needs additional test cases, but it performs worse in high degree,  and its need b

Second,  the graunette of the adaptiveness. Our frame work will one test cases at a time, and if . Different from our approach, this framework one covering array at a time, then identification, then additional test cases.
