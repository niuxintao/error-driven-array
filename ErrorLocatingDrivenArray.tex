...
\emph{\textbf{Q2}:} Does \emph{ICT} alleviates the three problems proposed in section \ref{sec:moti}. Specifically, $\textbf{Q2}_{a}$: does ICT reduce generating redundant and useless test cases , $\textbf{Q2}_{b}$ :does ICT reduce the appearance of test cases which contains multiple MFS,  and $\textbf{Q2}_{c}$ :does ICT reduce the impacts of masking effects.
...
\emph{\textbf{Q4}:}  Is it better to combine ICT with FDA-CIT.
...
\subsection{Does our approach alleviate the three problems}
...
Our result is shown in Fig.\ref{ex1_result}. This figure consists of nine sub-figures, one for each subject with specific testing coverage (ranged from 2 - 4 way). For each sub-figure, the x-axis represents the number of times a schema is covered in total, the y-axis represents the number of schemas.  For example in the first sub-figure (2-way for Tomcat), two bars with x-coordinate equal to 2 indicates that \emph{ICT} approach have 2.8 schemas which are covered twice and \emph{SCT} have 0.4 schemas. 

As we discussed previously, it is obviously that , so it is good that the bars are decent with the increase of x-axis. Hence our approach is

This is due to two modifications : reuse the test cases in identification. and forbidden the appearance of existed MFS to reduce the test cases that are used to identify the same MFS.

\begin{figure*}
 \includegraphics[width=7.0in]{ex1.eps}
\caption{Result of ex1}
\label{ex1_result}
\end{figure*}


in our result, it can maximal up to (not show in this figure)
...
\hline
Subject & Alg & 2-way       & 3-way       & 4-way       \\ \hline
Tomcat  & ict  & 0           & 0           & 0           \\
...
FDA-CIT \cite{yilmaz2013reducing} is a technique that. The overall process is to first . There are too main difference between our appraoch and its approach.

First, it use classification tree to. Differet from our OFOT approach, it does not needs additional test cases, but it perfomes worse in high degree,  and its need b

Second,  the graunette of the adaptiveness. Our frame work will one test cases at a time, and if . Different from our approach, this framework one covering array at a time, then identificatiuon, then additional test cases.

\subsubsection{Study setup}
We use FDA-CIT to identify.  We conunt the overall test cases (we donot sepately count the identiciation test cases and geernation test cases as FDA-CIT does not need to additional test cases to generate ), MFS identificaiton(including re-call, precise and f-measure), the t-tested-coverage, multiple MFS and the rate of schemas coverage.
...
To combine two approaches . However, not all combiation is good. For example, if two appraoches get the similar results. The combination of them does not imporvce the result, but will cost more as we does two approaches, inddicating more test cases are generated. Hence, we needs to the results of two aprpaoches as difference as possible. The ideal scenriao, is , one MFS cannot be idewnticied by this approach, but can identified by another approach.

\subsubsection{Study setup}
We count each MFS idneitifed by each approach. To count the . To measure the difference of two approaches, we introduce the ratio . that is . the.  which are .

The more the . the more differnece are , and idnicating the better the complemtray of two appraoches (»¥²¹ÐÔ). Latter we will compare the complentary from the combination of ICT and SCT with  the combination of ICT and FDA-CIT.
