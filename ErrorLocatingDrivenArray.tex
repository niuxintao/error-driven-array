...
Considering the large configuration space of HSQLDB, we chose CT to first generate a relatively small set of test cases. Each of them is actually a set of specific assignments to those options we cared. For each configuration, we tested HSQLDB by sending a excuting. We recorded the output of each run, but unfortunately, about half of them produced exceptions or warnings. Following the schedule of traditional CT, we started the identification process to isolate the failure-inducing option interactions of the configurations for those failing runs. Each failing run should be individually handled, in principle, as there may exist distinct failure-inducing option interactions among them. However, this successively identification process, although appealing, was hardly ever followed for this case study. This is because there are too many failing configurations and most of them contain the same failure-inducing option interactions, based on which it is wasteful and inefficient if applying MFS identification.

For the sake of convenience, we provide a highly simplified scenario of that testing as following. Consider we test the combinations of .  Possible values for each value is shown in Table \ref{tomcat-simplifiy}.
For this particular example. $t_{1}$ is . And $t$ and $t$ is a real faulty we cared \footnote{details see: http://sourceforge.net/p/hsqldb/bugs/981/}. That is if a test case contain these two interactions, or schemas will surely crashed.

\begin{table}[h]
\caption{Highly simplified configurations of HSQLDB}
\label{tomcat-simplifiy}
\center
\begin{tabular}{c|ccccc}
 \hline
 \bfseries Option & \multicolumn{5}{c}{\bfseries Values} \\  \hline
 jsp-config  & \multicolumn{5}{c}{ Values} \\
 jsp-config & \multicolumn{5}{c}{ " ", $$} \\
 Option & \multicolumn{5}{c}{ Values} \\
 Option & \multicolumn{5}{c}{ Values} \\
 \hline
\end{tabular}
\end{table}

 Before we figure this out, we  a 2-way covering array is generated in the \emph{generation} part of Table \ref{tradition-gi}. Note that every possible combinations of is covered by at least one test case. As expected, we conduct the identification for each individal failing test cases.


From this example there are following observations that is the limitations of the traditional CT process.


Like table, for \emph{generation} part, there are 9 test cases consists such a covering array. When we test them we found them fail. For this particalur seconerio, there are two MFS for these failures. Failure 1 is , i.e., . Failure 2 is .

There

When we study the . Hence, there are multiple failures in this . And which make many testing fail. As a consequent, we had to one by one , and unfortunately,
...
%\cline{2-7}
                                       ...
                                       & \multicolumn{5}{c|}{\bfseries test case}                                                                                                                                                                                                              & \bfseries Outcome \\ \cline{2-7}
                                       & \multicolumn{1}{l|}{$t_{1}$}   & \multicolumn{4}{l|}{0 \ \ \ \ 0 \ \ \ \  0\ \ \ \ 0}     & Fail              \\
                                       & \multicolumn{1}{l|}{$t_{2}$}   & \multicolumn{4}{l|}{0 \ \ \ \ 1 \ \ \ \  1\ \ \ \ 1}     & Pass              \\
                                       & \multicolumn{1}{l|}{$t_{3}$}   & \multicolumn{4}{l|}{0 \ \ \ \ 2 \ \ \ \  2\ \ \ \ 2}     & Pass              \\
                                       & \multicolumn{1}{l|}{$t_{4}$}   & \multicolumn{4}{l|}{1 \ \ \ \ 0 \ \ \ \  1\ \ \ \ 2}     & Fail              \\
                                       & \multicolumn{1}{l|}{$t_{5}$}   & \multicolumn{4}{l|}{1 \ \ \ \ 1 \ \ \ \  2\ \ \ \ 0}     & Pass              \\
                                       & \multicolumn{1}{l|}{$t_{6}$}   & \multicolumn{4}{l|}{1 \ \ \ \ 2 \ \ \ \  0\ \ \ \ 1}     & Pass              \\
                                       & \multicolumn{1}{l|}{$t_{7}$}   & \multicolumn{4}{l|}{2 \ \ \ \ 0 \ \ \ \  2\ \ \ \ 1}     & Fail              \\
                                       & \multicolumn{1}{l|}{$t_{8}$}   & \multicolumn{4}{l|}{2 \ \ \ \ 1 \ \ \ \  0\ \ \ \ 2}     & Pass              \\
                                       & \multicolumn{1}{l|}{$t_{9}$}   & \multicolumn{4}{l|}{2 \ \ \ \ 2 \ \ \ \  1\ \ \ \ 0}     & Pass              \\ \cline{2-7}
                                       \multicolumn{6}{c}{} \\
                                       & \multicolumn{6}{c}{\bfseries \emph{Identification}}                                                                                                                                                                                                                              \\ \cline{2-7}
\multicolumn{1}{l|}{\multirow{5}{*}{\rotatebox{90}{$t_{1}$ (0,0,0,0)}}} & \multicolumn{1}{l|}{$t_{10}$*} & \multicolumn{4}{l|}{1  \ \ \ \  0 \ \ \ \  0 \ \ \ \  0} & Fail              \\
\multicolumn{1}{l|}{}                  & \multicolumn{1}{l|}{$t_{11}$*} & \multicolumn{4}{l|}{0  \ \ \ \  1 \ \ \ \  0 \ \ \ \  0} & Pass              \\
\multicolumn{1}{l|}{}                  & \multicolumn{1}{l|}{$t_{12}$*} & \multicolumn{4}{l|}{0  \ \ \ \  0 \ \ \ \  1  \ \ \ \ 0} & Fail              \\
\multicolumn{1}{l|}{}                  & \multicolumn{1}{l|}{$t_{13}$*} & \multicolumn{4}{l|}{0  \ \ \ \  0 \ \ \ \  0  \ \ \ \ 1} & Fail              \\ \cline{2-7}
\multicolumn{1}{l|}{}                  & \multicolumn{1}{l|}{\bfseries MFS} & \multicolumn{5}{c}{\bfseries   (-, 0, -, -)}                                                                                                                                                                                                                 \\  \cline{2-7}
                                   %    \multicolumn{6}{c}{} \\
                                       %\cline{2-7}
\multicolumn{1}{l|}{\multirow{5}{*}{\rotatebox{90}{$t_{4}$ (1,0,1,2)}}} & \multicolumn{1}{l|}{$t_{14}$*} & \multicolumn{4}{l|}{2  \ \ \ \  0 \ \ \ \  1 \ \ \ \  2} & Fail              \\
\multicolumn{1}{l|}{}                  & \multicolumn{1}{l|}{$t_{15}$*} & \multicolumn{4}{l|}{1  \ \ \ \  1 \ \ \ \  1 \ \ \ \  2} & Fail              \\
\multicolumn{1}{l|}{}                  & \multicolumn{1}{l|}{$t_{16}$*} & \multicolumn{4}{l|}{1  \ \ \ \  0 \ \ \ \  2  \ \ \ \ 2} & Fail              \\
\multicolumn{1}{l|}{}                  & \multicolumn{1}{l|}{$t_{17}$*} & \multicolumn{4}{l|}{1  \ \ \ \  0 \ \ \ \  1  \ \ \ \ 0} & Fail              \\ \cline{2-7}
\multicolumn{1}{l|}{}                  & \multicolumn{1}{l|}{\bfseries MFS} & \multicolumn{5}{c}{\bfseries   (-, -, -, -)}                                                                                                                                                                                                                 \\  \cline{2-7}
                                      %  \multicolumn{6}{c}{} \\
                                %        \cline{2-7}
\multicolumn{1}{l|}{\multirow{5}{*}{\rotatebox{90}{$t_{7}$ (2,0,2,1)}}} & \multicolumn{1}{l|}{$t_{18}$*} & \multicolumn{4}{l|}{0  \ \ \ \  0 \ \ \ \  2 \ \ \ \  1} & Fail              \\
\multicolumn{1}{l|}{}                  & \multicolumn{1}{l|}{$t_{19}$*} & \multicolumn{4}{l|}{2  \ \ \ \  1 \ \ \ \  2 \ \ \ \  1} & Pass              \\
\multicolumn{1}{l|}{}                  & \multicolumn{1}{l|}{$t_{20}$*} & \multicolumn{4}{l|}{2  \ \ \ \  0 \ \ \ \  0  \ \ \ \ 1} & Fail              \\
\multicolumn{1}{l|}{}                  & \multicolumn{1}{l|}{$t_{21}$*} & \multicolumn{4}{l|}{2  \ \ \ \  0 \ \ \ \  2  \ \ \ \ 2} & Fail              \\ \cline{2-7}
\multicolumn{1}{l|}{}                  & \multicolumn{1}{l|}{\bfseries MFS} & \multicolumn{5}{c}{\bfseries   (-, 0, -, -)}                                                                                                                                                                                                                 \\  \cline{2-7}
\end{tabular}
\end{table}
...
Other MFS isolation was proposed to handle multiple MFS in one test case. But either cannot handle or needs too much extra test cases.

So in practice, we need to aviod the appreance. As to Sequet CT, however, does not give much support for this concern.
...
incorrect schemas can give us unrelieble results. However, this is inavoid. To definely get exact MFS, according to charle courblun and Nie , we need to exhaustive condunct every possible test cases, which is however, not possible. So in practice, we get an ½üËÆ½â [Zeller's paper to show how global minimal is not possible].

Based on this, if our algorithm get an wrong MFS, then it can be .


\emph{Possible solution}
1) Multiple approaches combined, this will enhance the Â³°ôÐÔ of our method. As we can learn in the emprically study, the combination of FDA-CI can significantly improve the prisce of the MFS identification and testing adquattely.
