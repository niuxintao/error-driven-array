...
\subsection{Details of the augmentation of the two CT activities}
...
$$t \leftarrow  Construct (\mathcal{P},\Omega ,  \xi) $$

The  three factors determine the. P determines the parameters and their values, which indicates a valid test case. $\Omega$ gives the uncovered schemas currently. Note that the is to . To the oppiste , each test case. It is noted that for a . Thus some random factor $xi$ is needed to the local optimization. 

This will transfer to the 


$$t \leftarrow  Construct (\mathcal{P},\Omega ,  \xi, \mathcal{M}) $$
...
$$t \leftarrow  mutant (\mathcal{P}, t_{o},\Delta ) $$
