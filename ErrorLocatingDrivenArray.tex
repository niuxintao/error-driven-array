...
This is mainly because we forbidden the appearance of MFS that has been identified. Consequently, as our approach progresses, the number of remaining MFS decreases one by one by one. Correspondingly, the probability that multiple MFS appear in one test case will also decrease. Since multiple MFS has a negative effect on MFS identification as discussed in Section \ref{sec:moti}, the reduction of the appearance of multiple MFS in one test case obviously improve the effectiveness of MFS identification.

(3)\emph{\textbf{The Masking effects is reduced, hence adequate testing is better satisfied}}

As discussed in Section \ref{sec:moti}, masking effects affects SCT when there are multiple MFS in one failing test case. Since our approach theoretically reduce the probability of such case, we believe our framework can support better to limit the masking effects. In fact, with our increment one by one, we can better determine which MFS are and which schemas are failure-irrelevant, which supports a better adequate testing than SCT.

\subsection{Example}\label{sec:app:example}

With this new framework, when we re-consider the example in Table \ref{tradition-gi} in Section \ref{sec:moti},  we can get the result listed in Table \ref{new-gi}.
\begin{table}[h]
...
Alougth our approach provides a more adaptive and flexible framework for CT, there are several issues that needs to be concerned.

\subsection{Incorrectly identified schemas}
incorrect schemas can give us unreliable results. However, this is inevitable. To definitely get exact MFS, according to charle courblun and Nie , we need to exhaustive condunct every possible test cases (based on the definition of MFS, every possible test cases contain this schema will fail), which is however, not possible. So in practice, we get an ½üËÆ½â [Zeller's paper to show how global minimal is not possible].
