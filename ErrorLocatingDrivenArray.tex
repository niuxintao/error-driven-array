...
In practice, it is impossible to thoroughly search the exhaustive candidate test cases $\mathcal{T}_{all}$ to get one test case $t$. Hence, some heuristic methods are used to simplify the selection. For example,  AETG \cite{cohen1997aetg} select each value to be . Correspondingly, approaches normally not , as a instead, they usually use a constraint solver to avoid the forbidden schemas, And like constraints handling Cohen's AETG-SAT \cite{cohen2007interaction,cohen2008constructing}.
