...
2) \textbf{More rigorous feedback validation}. This needs to cooperate with developers of the SUT. Specifically, when a MFS is identified. Not only should we send them to the developers, but also we need to get the feedback from them to see whether these MFS are the root cause of failure or not. Although this process may be time-consuming and delay the testing for remaining bugs, but it is the most effective way to determine the correct MFS .
