...
Now it is surely that this criteria is not satisfied with traditional covering array alone, next let us examine whether this criterion can be satisfied with SCT, i.e., too see if \emph{tested t-way interaction criterion} can be satisfied with the combination of traditional covering array and MFS identification. One obvious insight is that if there is only single MFS in each failing test case, this criterion is satisfied. For example in Table \ref{tradition-gi}, $t_{1}$ contained a single MFS (-, 0, -, -), and we identified this MFS by generating four extra test cases ($t_{10}$ to $t_{13}$). As for $t_{1}$, the schemas (-, 0, -, -) can be deemed to be covered. 
