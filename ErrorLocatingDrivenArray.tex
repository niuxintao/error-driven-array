...
The overall number of test cases generated by FDA-CIT for each subject is shown in Table \ref{num-fda-cit}. To better evaluate the performance of FDA-CIT, we list the gaps between FDA-CIT with ict and sct respectively in the parentheses (the first one is ict, latter one is sct). The computation of this gap between two approaches result is by the following formula.

$$ =  \frac{\#the\ num\ of\ correctly\ identified\  MFS}{\#the\ num\ of\ all\ the\ identified\ schemas}$$
...
The result is listed in Table \ref{tested-t-coverage-fda-cit}, the gaps between \emph{fda-cit} with \emph{ict} and \emph{sct} are listed in the parentheses, respectively (the first one is ict, latter one is sct). The computation is by . 

We can find that except subject Jflex (with single MFS) and Tcas (wit high-degree MFS) which all the three approaches work normally without masking effects, the schemas of other subjects doesn't get completely covered by the three approaches. Additionally, there doesn't exist apparent gaps between the three approaches. For example, fda-cit increase about 4.07\%  and 3.47\% 2-way tested-t-way coverage than ict and sct, respectively for the subject Hsqldb , but also decrease about 3.17\% and 0.02\%  4-way tested-t-way coverage for the subject Gcc. Considering the extent to which the fda-cit, fda-cit has a bit advantage (See Tomcat about), but in above, fda-cit has a advantages over, but is trivial.

This result gives us a confidence that our approach \emph{ict} does have level in limiting the masking effects when comparing with them. Another important to learn in this result, is that to limit the masking effects, give a adaptive framework to separately identify the MFS and forbidden them is not enough, to identify them accurately is important, and which should be the key in the further to the masking effects studies.
