...
The overall number of test cases generated by FDA-CIT for each subject is shown in Table \ref{num-fda-cit}. To better evaluate the performance of FDA-CIT, we list the gaps between FDA-CIT with ict and sct respectively in the parentheses (the first one is ict, latter one is sct). The computation of this gap between two approaches result is by .
...
2)\textbf{the F-measure of MFS identification} The results of the quality of MFS identification by FDA-CIT is listed in Table \ref{f-measure-fda-cit}. Then same as previous metric, the comparison result between fda-cit with \emph{ict} and \emph{sct} are also attached.
...
From this figure, we can learn that


4)\textbf{Masking effects}.

The results is listed in Table \ref{tested-t-coverage-fda-cit}.
